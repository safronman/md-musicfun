...
В `playlistApi` качестве демонстрации сломаем url. Например вот так:
...
Если же пользовательский код выбросит вместо обработанной, непредвиденную ошибку(`throw new Error`).
...
2. Логика довольно сложно читается 😢
...
Тут ответ будет немного сложнее. Бекенд возвращет ошибку согласно [JSON:API спецификации](https://jsonapi.org/)
...
Попробуйте передать невалидный `access token`. Подучите ошибку авторизации. Тут все просто, данный кейс уже написан в
429
...
Показывать пользователю только универсальное и безопасное сообщение, например:
