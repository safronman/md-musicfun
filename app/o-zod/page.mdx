...
<Callout type={"warning"}>
  Если бекенд ничего не возвращает, значит и валидировать ничего не нужно. Соответственно ендпоинты `deletePlaylist`,
...
Обратите внимание, что теперь нам в каждом ендпоинте где мы хотим сделать валидацию придется дублировать
`catchSchemaFailure`
...
Поэтому если вышеперечисленные аргументы для играют ключевую роль, то в production zod можо отключить при помощи
`skipschemavalidation`
...
**Локальное использование (для отельных эндпоинтов):**
