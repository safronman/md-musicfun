...
export const PlaylistCover = ({ images, playlistId }: Props) => {
  ...
  const uploadCoverHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      alert(`The file is too large. Max size is ${Math.round(maxSize / 1024)} КБ`)
    }
...
Заменим `alert` на красивую всплывашку

Установите библиотеку для работы с всплывашками [react-toastify](https://www.npmjs.com/package/react-toastify):
...
```tsx filename="PlaylistCover.tsx" {8, 12}
export const PlaylistCover = ({ images, playlistId }: Props) => {
  /*...*/
  ...
  const uploadCoverHandler = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast(`The file is too large (max. ${Math.round(maxSize / 1024)} КБ)`, { type: "error", theme: "colored" })
    }
...
**Результат:** получаем красивую вплывашку 🚀
