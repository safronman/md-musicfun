...
Давайте сразу заложим базовый роутинг и создадим страницы, которые понадобятся в процессе разработки приложения:
...
- 404 страницу(`PageNotFound.tsx`) и добавим их в роутинг.

<Callout type={"warning"}>
  В качестве структуры папок рекомендациями которые предлагает `RTK`:
