...
Автоматически сгенерированные `RTK Query` хуки предоставляют производные булевые значения
...
Сейчас скелетон мы не будем реализовывать, т.к. на верстку сейчас мы акцент не делаем. Но в качетсве домашнего
...
А при пагинации свойство `isFetching` и показывать `LinearProgress`
...
Соотвественно хотелось бы иметь какой-то глобальный loading.

Существует несколько вариантов, чтобы реализовать такой loading:
...
**Результат:** теперь при каждом запросе будет показываться `LinearProgress` 🚀

### Исключения ендпоинтов

Если для некоторых ендпоинтов мы хотим сделать исключения, то просто будем добавлять их в массив `excludedEndpoints`
